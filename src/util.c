...
int DisplayBanner(void) {
	fprintf(stderr,
			"\n"            " /\\/\\  \n"
					" ()()   -*>       Squirrel <*-\n"
					" {..}   ,,        Version %s.%s.%s (Build %s)%s%s\n"
					" (  )~~~//        By UnionSmartIT : http://www.unionsmartit.com/\n"
					"  && ~~~/         (C) Copyright 2016 usi01 <usi01@unionsmartit.com>\n"
...
void ts_print(register const struct timeval *tvp, char *timebuf) {
	...
	if (BcOutputIncludeYear()) {
		(void) SnortSnprintf(timebuf, TIMEBUF_SIZE,
				"%02d/%02d/%02d-%02d:%02d:%02d.%06u ", lt->tm_mon + 1,
				lt->tm_mday, lt->tm_year - 100, s / 3600, (s % 3600) / 60,
				s % 60, (u_int) tvp->tv_usec);
	} else {
		(void) SnortSnprintf(timebuf, TIMEBUF_SIZE,
				"%02d/%02d-%02d:%02d:%02d.%06u ", lt->tm_mon + 1, lt->tm_mday,
...
void ts_print2(uint32_t sec, uint32_t usec, char *timebuf) {
	...
	if (BcOutputIncludeYear()) {
		...
		(void) SnortSnprintf(timebuf, TIMEBUF_SIZE,
				"%02d/%02d-%02d:%02d:%02d.%06u ", lt->tm_mon + 1, lt->tm_mday,
				s / 3600, (s % 3600) / 60, s % 60, (u_int) usec);
	}
}
	...
	if (1)	// TODO: Read mode flag
	{
		...
		if (strlen(barnyard2_conf->pid_path) != 0) {
			if ((stat(barnyard2_conf->pid_path, &pt) == -1)
					|| !S_ISDIR(pt.st_mode)
					|| access(barnyard2_conf->pid_path, W_OK) == -1) {
#ifndef WIN32
				...
				if (err) {
					LogMessage("Previous Error, errno=%d, (%s)\n", err,
							strerror(err) == NULL ?
									"Unknown error" : strerror(err));
				}
#endif
				memset(barnyard2_conf->pid_path, '\0',
						sizeof(barnyard2_conf->pid_path));
			} else {
...
# endif  /* WIN32 */
			...
			if (!S_ISDIR(pt.st_mode) || access(_PATH_VARRUN, W_OK) == -1) {
				LogMessage("WARNING: _PATH_VARRUN is invalid, trying "
						"/var/log...\n");
				SnortStrncpy(barnyard2_conf->pid_path, "/var/log/",
						sizeof(barnyard2_conf->pid_path));
				...
				if (!S_ISDIR(pt.st_mode)
					...
					LogMessage("WARNING: %s is invalid, logging Snort "
							...
							barnyard2_conf->pid_path, barnyard2_conf->log_dir);
					CheckLogDir();
					SnortSnprintf(barnyard2_conf->pid_path,
							sizeof(barnyard2_conf->pid_path), "%s/",
							barnyard2_conf->log_dir);
				}
			} else {
				LogMessage("PID path stat checked out ok, "
						"PID path set to %s\n", _PATH_VARRUN);
				SnortStrncpy(barnyard2_conf->pid_path, _PATH_VARRUN,
						sizeof(barnyard2_conf->pid_path));
			}
		}
	}
	...
	SnortSnprintf(barnyard2_conf->pid_filename,
			sizeof(barnyard2_conf->pid_filename), "%s/barnyard2_%s%s.pid",
			barnyard2_conf->pid_path, intf, barnyard2_conf->pidfile_suffix);
	...
	if (pid_file) {
		...
		ErrorMessage("Failed to create pid file %s",
				barnyard2_conf->pid_filename);
...
void DropStats(int exiting) {
	...
	LogMessage("      ETH: " FMTu64("-10") " (%.3f%%)\n", pc.eth,
			CalcPct(pc.eth, total));
	LogMessage("  ETHdisc: " FMTu64("-10") " (%.3f%%)\n", pc.ethdisc,
			CalcPct(pc.ethdisc, total));
#ifdef GIDS
#ifndef IPFW
...
void GoDaemon(void) {
	...
	open("/tmp/barnyard2.debug", O_CREAT | O_RDWR, S_IRWXU); /* stdin, fd 0 */
...
void SignalWaitingParent(void) {
	...
	if (kill(parentpid, SIGNAL_SNORT_CHILD_READY)) {
		LogMessage(
				"Daemon initialized, failed to signal parent pid: %d, failure: %d, %s\n",
...
void SetChroot(char *directory, char **logstore) {
	...
	*logstore = NULL;
	...
	DEBUG_WRAP(DebugMessage(DEBUG_INIT,"chroot success (%s ->", absdir);); DEBUG_WRAP(DebugMessage(DEBUG_INIT,"%s)\n ", CurrentWorkingDir()););
...
int Move(const char *source, const char *dest) {
	if (link(source, dest) != 0) {
		...
		LogMessage("Failed to archive file \"%s\" to \"%s\": %s", source, dest,
				strerror(errno));
	} else {
		if (unlink(source) != 0) { /* oops, unlink/remove has failed */
			LogMessage("Failed to unlink \"%s\": %s", source, strerror(errno));
		}
	}
	return 0;
}
...
char *GetUniqueName(char * iface) {
	...
	if (iface == NULL)
		LogMessage("Interface is NULL. Name may not be unique for the host\n");
	if (iface != NULL) {
	...
	if (BcLogVerbose())
		LogMessage("Node unique name is: %s\n", rptr);
	return rptr;
}
...
char *GetTimestampByComponent(uint32_t sec, uint32_t usec, int tz) {
	...
	if (BcOutputUseUtc()) {
		...
		SnortSnprintf(buf, SMALLBUFFER, "%04i-%02i-%02i %02i:%02i:%02i.%03i",
				...
				lt->tm_min, lt->tm_sec, msec);
	} else {
		lt = localtime(&Time);
		SnortSnprintf(buf, SMALLBUFFER,
				"%04i-%02i-%02i %02i:%02i:%02i.%03i+%03i", 1900 + lt->tm_year,
...
u_int32_t GetTimestampByComponent_STATIC(uint32_t sec, uint32_t usec, int tz,
		char *buf) {
	struct tm *lt; /* localtime */
	time_t Time;
	int msec;
...
u_int32_t GetTimestampByStruct_STATIC(register const struct timeval *tvp,
		int tz, char *buf) {
	struct tm *lt; /* localtime */
	int msec;
...
char * base64(const u_char * xdata, int length) {
	...
	output = (char *) SnortAlloc(
			((unsigned int) (length * 1.5 + 4)) * sizeof(char));
