...
typedef enum _GetOptArgType {
    LONGOPT_ARG_NONE = 0, LONGOPT_ARG_REQUIRED, LONGOPT_ARG_OPTIONAL
...
OutputFuncNode *FlushList = NULL; /* Flush function list */
...
static Barnyard2Config * MergeBarnyard2Confs(Barnyard2Config *,
        Barnyard2Config *);
...
int main(int argc, char *argv[])
{
    ...
    return SquirrelMain(barnyard2_argc, barnyard2_argv);
}

uint8_t by_openlock(const char *l_file, int *pfd)
{
    int rc;
    struct flock by_flock;

    *pfd = open(l_file, O_CREAT|O_RDWR, 0);
    if ( *pfd < 0 ) {
    	return 1;
    }

    memset(&by_flock, 0, sizeof(by_flock));
    by_flock.l_type = F_RDLCK | F_WRLCK;
    by_flock.l_whence = SEEK_SET;

    rc = fcntl(*pfd, F_SETLK, &by_flock);
    if (rc == -1) {
    	perror("fcntl F_RDLCK | F_WRLCK conflict.\n");
    	close(*pfd);
    	return 1;
    }
...
uint8_t by_closelock(int *pfd)
{
    int rc;
    struct flock by_flock;

    memset(&by_flock, 0, sizeof(by_flock));
    by_flock.l_type = F_UNLCK;
    by_flock.l_whence = SEEK_SET;
...
/*
 *
 * Function: SquirrelMain(int, char *)
 *
...
int SquirrelMain(int argc, char *argv[])
{
    ...
    if ( 0 != by_openlock(barnyard2_conf->waldo_filepath, &lock_fd) ) {
    	LogMessage("%s: Get file lock failed.\n", __func__);
    	return 1;
    }

    LogMessage("%s: Get file lock OK.\n", __func__);

    Barnyard2PostInit();

    /* check for waldo file usage */
    if (barnyard2_conf->waldo_state & WALDO_STATE_ENABLED) {
        /*		int				ret;
    ...
    /* Continual processing mode */
        ...
        ProcessContinuousWithWaldo(barnyard2_conf);

        LogMessage("%s: Exiting, Signal Check", __func__);
...
static void InitPidChrootAndPrivs(void)
{
    ...
    if (BcDaemonMode() || *barnyard2_conf->pidfile_suffix
            || BcCreatePidFile()) {
#ifdef WIN32
        CreatePidFile("WIN32");
#else            
...
static void ParseCmdLine(int argc, char **argv)
{
    ...
    for (i = 0; i < argc; i++) {
        if (strcmp("-M", argv[i]) == 0) {
            ...
            openlog("squirrel", LOG_PID | LOG_CONS, LOG_DAEMON);
    ...
    while ((ch = getopt_long(argc, argv, valid_options, long_options,
            &option_index)) != -1) {
        ...
        case 'd': /* dump the application layer data */
            ConfigSpoolDirectoryTr(bc, optarg, 0);
            break;
        ...
        case 'r': /* augment pid file name suffix */
            ...
            SnortStrncpy(bc->pidfile_suffix, optarg,
                    sizeof(bc->pidfile_suffix));
            break;
...
//              break;
     ...
     * don't match */
    else if ((bc->run_mode_flags & RUN_MODE_FLAG__CONTINUOUS)
            && (barnyard2_conf_file != NULL)) {
    ...
    if (((bc->run_mode == RUN_MODE__CONTINUOUS)
            || (bc->run_mode == RUN_MODE__BATCH)
            || (bc->run_mode == RUN_MODE__TEST))
            && (barnyard2_conf_file == NULL)) {
...
static void Barnyard2Cleanup(int exit_val, int exit_needed)
{
    ...
    if (exit_needed) {
        exit_signal = SIGQUIT;
        sleep(1);   //Wait the other threads exit, 1 second should be enough
    }
    /******* Multiple Thread support end********/
    ...
    for (i=0; i<BY_MUL_TR_DEFAULT; i++) {
        if ( NULL != barnyard2_conf->spooler[i] ) {
            spoolerEventCacheFlush(barnyard2_conf->spooler[i]);
            spoolerClose(barnyard2_conf->spooler[i]);
            barnyard2_conf->spooler[i] = NULL;
        ...
        spoolerCloseWaldo(&(barnyard2_conf->waldos[i]));
    }
    ...
    /* remove pid file */
    if (SnortStrnlen(barnyard2_conf->pid_filename,
            sizeof(barnyard2_conf->pid_filename)) > 0) {
        int ret;
...
{
    Barnyard2Config *bc = (Barnyard2Config *) SnortAlloc(
            sizeof(Barnyard2Config));
...
static void SetBarnyard2ConfDir(void)
{
    ...
    if (barnyard2_conf_file != NULL) {
        ...
        if (path_sep != NULL) {
            ...
            barnyard2_conf_dir = SnortStrndup(barnyard2_conf_file,
                    path_sep - barnyard2_conf_file);
        } else {
...
static Barnyard2Config * MergeBarnyard2Confs(Barnyard2Config *cmd_line,
        Barnyard2Config *config_file)
{
    ...
    if (cmd_line == NULL) {
        FatalError(
                "%s(%d) Merging barnyard2 configs: barnyard2 conf is NULL.\n",
                __FILE__, __LINE__);
    }
    ...
    if ((cmd_line->sid_msg_file) && (config_file->sid_msg_file)) {
        FatalError(
                "The sid map file was included two times command line (-S) [%s] and in the configuration file (config sid_map) [%s].\n"
                        "It only need to be defined once.\n",
                cmd_line->sid_msg_file, config_file->sid_msg_file);
    }

    if ((cmd_line->gen_msg_file) && (config_file->gen_msg_file)) {
        FatalError(
                "The gen map file was included two times command line (-G) [%s] and in the configuration file (config gen_map) [%s].\n"
                        "It only need to be defined once.\n",
                cmd_line->gen_msg_file, config_file->gen_msg_file);
    }
    ...
    if (config_file->alert_on_each_packet_in_stream_flag == 0) {
        LogMessage(
                "[INFO]: Alerting on each packet in stream has been disabled by configuration file,\n"
        ...
        LogMessage(
                "[INFO]: Alerting on each packet in stream has been disabled by command line option,\n"
                        "\tevents will only be outputed for the first matching event/packet,\n"
                        "\tfurther packets matching previous processed events will be ignored");

        config_file->alert_on_each_packet_in_stream_flag =
                cmd_line->alert_on_each_packet_in_stream_flag;
    }

    config_file->process_new_records_only_flag =
            cmd_line->process_new_records_only_flag;
    ...
    if (cmd_line->spool_filebase[0] != '\0') {
        ConfigSpoolFilebase(config_file, cmd_line->spool_filebase);
    }

    if (cmd_line->waldo_filepath[0] != '\0') {
        ConfigWaldoFile(config_file, cmd_line->waldo_filepath);
    }
...
static void Barnyard2Init(int argc, char **argv)
{
    ...
    LogMessage("\n");
    LogMessage("        --== Initializing Squirrel ==--\n");
...
//        DumpPreprocessors();
        ...
        LogMessage("Squirrel spooler: Event cache size set to [%u] \n",
    ...
    if (barnyard2_conf->sidmap_version == SIDMAPV2) {
        if (SignatureResolveClassification(barnyard2_conf->classifications,
                (SigNode *) *BcGetSigNodeHead(), barnyard2_conf->sid_msg_file,
                barnyard2_conf->class_file)) {
            FatalError(
                    "[%s()], Call to SignatureResolveClassification failed \n",
                    __FUNCTION__);
        }
    }
...
static void Barnyard2PostInit(void)
{
    ...
    if (BcDaemonMode()) {
        LogMessage("Squirrel initialization completed successfully (pid=%u)\n",
                getpid());
    }
