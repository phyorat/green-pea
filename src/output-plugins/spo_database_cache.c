 ...
 * 1 FOUND
 */
u_int32_t cacheSignatureReferenceLookup(dbSignatureReferenceObj *iLookup,
		cacheSignatureReferenceObj *iHead) {
	if ((iLookup == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbReferenceSignatureObj[0x%x] cacheSignatureReferenceObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
...
u_int32_t cacheEventSignatureLookup(cacheSignatureObj *iHead,
	...
	if ((iHead == NULL) || (sigContainer == NULL)) {
		return 0;
	}
	...
	while (iHead != NULL) {
		if ((iHead->obj.sid == sid) && (iHead->obj.gid == gid)) {
...
u_int32_t cacheSignatureLookup(dbSignatureObj *iLookup,
	...
	if ((iLookup == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbSignatureObj[0x%x] cacheSignatureObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
 ...
 * Valid POINTER  FOUND
 */
cacheSignatureObj * cacheSignatureGetObject(dbSignatureObj *iLookup,
		cacheSignatureObj *iHead) {
...
u_int32_t cacheClassificationLookup(dbClassificationObj *iLookup,
	...
	if ((iLookup == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbClassiciationObj[0x%x] cacheClassificationObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
...
u_int32_t cacheSystemLookup(dbSystemObj *iLookup, cacheSystemObj *iHead,
	...
	if ((iLookup == NULL) || (rcacheSystemObj == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbReferenceObj[0x%x] cacheReferenceObj[0x%x] **rcacheSystemObj[0x%x]\n",
				__FUNCTION__, iLookup, iHead, rcacheSystemObj);
	}

	while (iHead != NULL) {
		if ((memcmp(iLookup->ref_system_name, iHead->obj.ref_system_name,
				SYSTEM_NAME_LEN) == 0)
				&& (memcmp(iLookup->ref_system_url, iHead->obj.ref_system_url,
						SYSTEM_URL_LEN) == 0)) {
			/* Match */
...
u_int32_t cacheReferenceLookup(dbReferenceObj *iLookup,
		cacheReferenceObj *iHead, cacheReferenceObj **retRefLookupNode) {
	if ((iLookup == NULL) || (retRefLookupNode == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbReferenceObj[0x%x] cacheReferenceObj[0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
...
u_int32_t dbSignatureReferenceLookup(dbSignatureReferenceObj *iLookup,
	...
	if ((iLookup == NULL) || (retSigRef == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbReferenceSignatureObj[0x%x] cacheSignatureReferenceObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
...
u_int32_t dbSystemLookup(dbSystemObj *iLookup, cacheSystemObj *iHead) {
	if ((iLookup == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbSystemObj[0x%x] cacheSystemObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
...
u_int32_t dbClassificationLookup(dbClassificationObj *iLookup,
	...
	if ((iLookup == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], Called with dbReferenceObj[0x%x] cacheClassificationObj [0x%x] \n",
				__FUNCTION__, iLookup, iHead);
	}
...
u_int32_t ConvertReferenceCache(ReferenceNode *iHead, MasterCache *iMasterCache,
	...
	if ((iMasterCache == NULL) || (cSobj == NULL)) {
		/* XXX */
		return 1;
	}
	...
	while (cNode != NULL) {
		...
		if (cNode->system != NULL) {
			strncpy(sys_LobjNode.ref_system_name, cNode->system->name,
					SYSTEM_NAME_LEN);
		...
		if (cNode->system->url != NULL) {
			strncpy(sys_LobjNode.ref_system_url, cNode->system->url,
					SYSTEM_URL_LEN);
		...
		if (cacheSystemLookup(&sys_LobjNode, iMasterCache->cacheSystemHead,
				&sysRetCacheNode) == 0) {
		...
		if (sysRetCacheNode != NULL) {
			...
			if ((cacheReferenceLookup(&ref_LobjNode,
					sysRetCacheNode->obj.refList, &retRefLookupNode) == 0)) {
				if ((ref_TobjNode = SnortAlloc(sizeof(cacheReferenceObj)))
						== NULL) {
					/* XXX */
					return 1;
				}
				...
				for (tItr = 0; tItr < cSobj->obj.ref_count; tItr++) {
					if ((memcmp(&cSobj->obj.ref[tItr]->obj,
							&retRefLookupNode->obj, sizeof(dbReferenceObj)))
							== 0) {
						refFound = 1;
						break;
					}
		...
		DatabaseData *data)
{
	...
	while (cNode != NULL) {
		...
		if ((cacheSignatureLookup(&lookupNode, iMasterCache->cacheSignatureHead)
				== 0)) {
			if ((TobjNode = SnortAlloc(sizeof(cacheSignatureObj))) == NULL) {
				/* XXX */
				return 1;
			...
			LogMessage(
					"WARNING: While processing data parsed from SIGNATURE FILE a duplicate entry was found [DUPLICATE ARE NOT PROCESSED]:\n"
							...
							"\tMessage  [%s]\n", lookupNode.gid, lookupNode.sid,
					lookupNode.rev, lookupNode.class_id, lookupNode.message);
		}
...
u_int32_t ConvertClassificationCache(ClassType **iHead,
	...
	if ((iHead == NULL) || (iMasterCache == NULL)
	...
	if ((cNode = *iHead) == NULL) {
		LogMessage(
				"[%s()], No classification was found in the classification file,\n"
	...
	while (cNode != NULL) {
		...
		if (cNode->type != NULL) {
			...
			snprintf(LobjNode.obj.sig_class_name, CLASS_NAME_LEN,
					"[%s] id:[%u]", "UNKNOWN SNORT CLASSIFICATION",
					LobjNode.obj.sig_class_id);
		}

		if ((cacheClassificationLookup(&LobjNode.obj,
				iMasterCache->cacheClassificationHead) == 0)) {
				...
				while ((curr_row < num_row)
					...
					for (i = 0; i < queryColCount; i++) {
						...
						if (row[i]) {
							switch (i) {
							case 0:
								cPtr->db_sig_class_id = strtoul(row[i], NULL,
										10);
								break;

							case 1:
								strncpy(cPtr->sig_class_name, row[i],
										CLASS_NAME_LEN);
								...
								if ((snort_escape_string_STATIC(cPtr->sig_class_name, data->sanitize_buffer[0],
									...
									FatalError(
											"database [%s()], Failed a call to snort_escape_string_STATIC() for string : \n"
													"[%s], Exiting. \n",
											__FUNCTION__, cPtr->sig_class_name);
								}
...
u_int32_t ClassificationCacheSynchronize(DatabaseData *data,
	...
	if (array_length > 0) {
		if ((ClassificationCacheUpdateDBid(dbClassArray, array_length,
			...
			LogMessage(
					"[%s()], Call to ClassificationCacheUpdateDBid() failed \n",
					__FUNCTION__);
			return 1;
		}
	...
	if (*cacheHead != NULL) {
		if (ClassificationPopulateDatabase(data, *cacheHead)) {
			LogMessage(
					"[%s()], Call to ClassificationPopulateDatabase() failed \n",
					__FUNCTION__);
...
u_int32_t SignaturePullDataStore(DatabaseData *data, dbSignatureObj **iArrayPtr,
	...
	if ((data == NULL) || ((iArrayPtr == NULL) && (*iArrayPtr != NULL))
		...
		/* XXX */
		LogMessage(
				"[%s()], Call failed DataBaseData[0x%x] dbSignatureObj **[0x%x] u_int32_t *[0x%x] \n",
				__FUNCTION__, data, iArrayPtr, array_length);
		return 1;
	}
...
u_int32_t SignatureReferencePreGenerate(cacheSignatureObj *iHead) {
	...
	while (cObj != NULL) {
		if ((cObj->flag & CACHE_BOTH) && (cObj->obj.rev != 0)
				&& (cObj->obj.ref_count > 0)) {
			searchObj = iHead;

			while (searchObj != NULL) {
				if ((searchObj != cObj) && (cObj->obj.sid == searchObj->obj.sid)
					...
					memcpy(searchObj->obj.ref, cObj->obj.ref,
							(sizeof(cacheReferenceObj *) * MAX_REF_OBJ));
				}
...
u_int32_t ReferencePullDataStore(DatabaseData *data, dbReferenceObj **iArrayPtr,
	...
	if ((data == NULL) || ((iArrayPtr == NULL) && (*iArrayPtr != NULL))
		...
		/* XXX */
		LogMessage(
				"[%s()], Call failed DataBaseData[0x%x] dbSystemObj **[0x%x] u_int32_t *[0x%x] \n",
				__FUNCTION__, data, iArrayPtr, array_length);
		return 1;
	}
				...
				while ((curr_row < num_row)
					...
					for (i = 0; i < queryColCount; i++) {
						...
						if ((row[i] != NULL)) {
							...
							case 0:
								cPtr->db_ref_system_id = strtoul(row[i], NULL,
										10);
								break;

							case 1:
								strncpy(cPtr->ref_system_name, row[i],
										SYSTEM_NAME_LEN);
								cPtr->ref_system_name[SYSTEM_NAME_LEN - 1] =
										'\0'; //toasty.
...
u_int32_t SystemCacheSynchronize(DatabaseData *data, cacheSystemObj **cacheHead) {
	...
	while (SystemCacheElemPtr != NULL) {
		if (SystemCacheElemPtr->obj.refList != NULL) {
			if (ReferencePopulateDatabase(data,
					SystemCacheElemPtr->obj.refList)) {
				LogMessage(
						"[%s()], Call to ReferencePopulateDatabase() failed \n",
						__FUNCTION__);
				goto func_fail;
			}
		}
...
u_int32_t GenerateSigRef(cacheSignatureReferenceObj **iHead,
	...
	while (sigHead != NULL) {
		...
		if (sigHead->obj.db_id != 0) {
			for (node_count = 0; node_count < sigHead->obj.ref_count;
					node_count++) {
				...
				if ((cacheSignatureReferenceLookup(&lookupNode, *iHead)) == 0) {
					if ((newNode = SnortAlloc(
							sizeof(cacheSignatureReferenceObj))) == NULL) {
						/* XXX */
						return 1;
					}
...
u_int32_t SignatureReferenceCacheUpdateDBid(dbSignatureReferenceObj *iDBList,
	...
	if ((iDBList == NULL) || (cacheHead == NULL) || (sigCacheHead == NULL)
			|| (systemCacheHead == NULL) || (array_length == 0)) {
		/* XXX */
		return 1;
	}
	...
	for (x = 0; x < array_length; x++) {
		...
		if (tempCache != NULL) {
			if ((dbSignatureReferenceLookup(cObj, tempCache, &rNode, 0) == 0)) {
				if ((tNode = SnortAlloc(sizeof(cacheSignatureReferenceObj)))
						== NULL) {
					/* XXX */
					goto f_err;
				}
				...
				LogMessage(
						"Warning [%s()] : sig_id [%u] ref_id [%u] ref_seq [%u] Duplicate found in database with database constraint? Ignoring element in temporary cache \n",
						cObj->db_sig_id, cObj->db_ref_id, cObj->ref_seq);
			}
		} else {
	...
	while (cacheLookup != NULL) {
		...
		if (dbSignatureReferenceLookup(&cacheLookup->obj, tempCache, &rNode,
				1)) {
			if ((cacheLookup->obj.ref_seq != rNode->obj.ref_seq)) {
				...
				if (cacheLookup->obj.ref_seq > MAX_REF_OBJ) {
					FatalError(
							"[%s()], can't process reference_sequence of [%d] for signature [%d] reference [%d] \n",
							__FUNCTION__, cacheLookup->obj.ref_seq,
...
u_int32_t SigRefSynchronize(DatabaseData *data,
	...
	if (array_length > 0) {
		if ((SignatureReferenceCacheUpdateDBid(dbSigRefArray, array_length,
			...
			LogMessage(
					"[%s()], Call to SignatureReferenceCacheUpdateDBid() failed \n",
					__FUNCTION__);
			return 1;
		}
...
u_int32_t ConvertDefaultCache(Barnyard2Config *bc, DatabaseData *data) {
	if ((bc == NULL) || (data == NULL)) {
		/* XXX */
		FatalError(
				"database [%s()], received a NULL argument : Barnyard2Config [0x%x] or DatabaseData [0x%x]  \n",
				__FUNCTION__, bc, data);
	}
...
void MasterCacheFlush(DatabaseData *data, u_int32_t flushFlag) {
	...
	if ((flushFlag & CACHE_FLUSH_SIGREF)
		...
		while (MCcacheSignature != NULL) {
			...
			memset(MCcacheSignature->obj.ref, '\0',
					(sizeof(cacheReferenceObj *) * MAX_REF_OBJ));
