...
void LogAscii(Packet *, void *, uint32_t, void *);
...
void LogAscii(Packet *p, void *event, uint32_t event_type, void *arg)
{
    FILE *log_ptr = NULL;
	SigNode				*sn = NULL;
    ...
    if(sn != NULL)
    {
         ...
         */
        if(  !strstr(sn->msg,"[**]") && !strchr(sn->msg,'\n') )
        {
          fwrite(sn->msg, strlen(sn->msg), 1, log_ptr);
        }
        fwrite(" [**]\n", 6, 1, log_ptr);
    }
    if(p)
...
FILE *OpenLogFile(int mode, Packet * p)
{
    ...
    {
        SnortSnprintf(log_file, STD_BUF, "%s/%s", barnyard2_conf->log_dir, logfile[mode]);
...
#ifdef SUP_IP6
    ip = GET_DST_IP(p);
    if(sfip_contains(&barnyard2_conf->homenet, ip) == SFIP_CONTAINS)
#else
    if((p->iph->ip_dst.s_addr & barnyard2_conf->netmask) == barnyard2_conf->homenet)
#endif
    {
#ifdef SUP_IP6
        if(sfip_contains(&barnyard2_conf->homenet, ip) == SFIP_CONTAINS)
#else
        if((p->iph->ip_src.s_addr & barnyard2_conf->netmask) != barnyard2_conf->homenet)
#endif
        {
            SnortSnprintf(log_path, STD_BUF, "%s/%s", barnyard2_conf->log_dir, 
