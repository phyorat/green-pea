...
static const ConfigFunc config_opts[] =
{
    ...
    { CONFIG_OPT__SPOOL_DIR, 1, 0, ConfigSpoolDirectory },
    { CONFIG_OPT__SPOOL_FILEBASE, 1, 1, ConfigSpoolFilebase },
...
void ConfigSpoolFilebase(Barnyard2Config *bc, char *args)
{
    ...
    if ( SnortSnprintf(bc->spool_filebase, STD_BUF, "%s", args) != SNORT_SNPRINTF_SUCCESS )
...
void ConfigSpoolDirectoryTr(Barnyard2Config *bc, char *args, uint8_t t_index)
{
    ...
    if ( t_index >= BY_MUL_TR_DEFAULT ) {
        FatalError("barnyard2: invalid sub-thread index\n");
        return;
    }

    if ( SnortSnprintf(bc->waldos[t_index].data.spool_dir, STD_BUF, "%s%02d", args, t_index+1) != SNORT_SNPRINTF_SUCCESS )
...
void ConfigWaldoFile(Barnyard2Config *bc, char *args)
{
    ...
    if ( SnortSnprintf(bc->waldo_filepath, STD_BUF, "%s", args) != SNORT_SNPRINTF_SUCCESS )
        FatalError("barnyard2: waldo filepath too long\n");

    bc->waldo_state |= WALDO_STATE_ENABLED;
}
